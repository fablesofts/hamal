...
//    may close an IDSESSION connection too if you don't follow the protocol's requirements.
    ...
    public String cmd(byte[] cmd){
        ...
        try {  // finally to close resources
            ...
            try {
                ...
                log.info("error getting InputStream from socket", e);
                return response;
            }
            ...
            while (read == DEFAULT_CHUNK_SIZE){
                try {
                    ...
                    log.info("error reading result from socket", e);
                    break;
                }
            ...
            if (dos != null) try { dos.close(); } catch (IOException e) { log.info("exception closing DOS", e); }
            try { socket.close(); } catch (IOException e) { log.info("exception closing socket", e); }
        }
     ...
     * @throws IOException
     */
    public String scan(byte[] in) throws IOException {
    ...
    public String scan(InputStream in) {
        ...
        try {
            ...
            log.info("could not connect to clamd server", e);
            return null;    
        }
        ...
        try {
        	...
        	log.info("Could not set socket timeout to " + getTimeout() + "ms", e);        
        }
        ...
        try {  // finally to close resources
            ...
            try {
                ...
                log.info("could not open socket OutputStream", e);
                return null;
            }
            ...
            try {
                ...
                log.info("error writing INSTREAM command", e);
                return null;
            }
            ...
            while (read == DEFAULT_CHUNK_SIZE){
                ...
                try {
                    ...
                    log.info("error writing data to socket", e);
                    break;
                }
            }
            ...
            try {
                ...
                log.info("error writing zero-length chunk to socket", e);
            }
        ...
        if (log.isDebugEnabled()) log.info( "Response: " + response);
        return ScanResult.setResult(response.trim());
	}
