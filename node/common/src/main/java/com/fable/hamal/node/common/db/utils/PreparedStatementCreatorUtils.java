...
/**
 * 从Spring的{@link StatementCreatorUtils}拷贝修改处理字段类型匹配问题
 * @author xieruidong 2014年5月12日 下午5:18:11
 */
...
public abstract class PreparedStatementCreatorUtils {
	...
	private static final Log logger = LogFactory.getLog(PreparedStatementCreatorUtils.class);
	...
	private static void setValue(PreparedStatement ps, int paramIndex, int sqlType, String typeName,
		...
		}//增加NCHAR,NVARCHAR,LONGNVARCHAR,NCLOB处理
		else if (sqlType == Types.NCHAR && isStringValue(inValue.getClass())) {
			ps.setNString(paramIndex, inValue.toString());
		}
		else if (sqlType == Types.NVARCHAR && isStringValue(inValue.getClass())) {
			ps.setNString(paramIndex, inValue.toString());
		}
		else if (sqlType == Types.LONGNVARCHAR && isStringValue(inValue.getClass())) {
			ps.setNString(paramIndex, inValue.toString());
		}
		else if (sqlType == Types.NCLOB) {
			ps.setNClob(paramIndex, (NClob)inValue);
		}
