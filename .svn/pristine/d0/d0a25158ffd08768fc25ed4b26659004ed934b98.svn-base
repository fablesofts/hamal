 ...
 * @author zhangl
 */
public class AddFPTableLoaderFactory extends AbstractLoaderFactory implements LoaderFactory {
	...
	public List<Loader> createLoaders(Pump pump) {
		...
		for (Pair pair : pump.getPairs()) {
			AddFPTableLoader loader = new AddFPTableLoader();
			...
			loader.setDbDialect(sqldbDialectFactory.getDbDialect(pump.getId(), dcs));
			result.add(loader);
		}
		return result;
	}
